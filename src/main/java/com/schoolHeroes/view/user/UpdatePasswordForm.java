	...
	private void updatePasswordListernerList(final User user) {
		...
		updatePasswordButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (oldPasswordField.getPassword().length == 0 || newPasswordField.getPassword().length == 0
						|| confirmNewPasswordField.getPassword().length == 0) {
				...
				if (!Arrays.equals(newPasswordField.getPassword(), confirmNewPasswordField.getPassword())) {
					JOptionPane.showMessageDialog(UpdatePasswordForm.this,
							"Les deux mots de passe ne correspondent pas !", "Erreur", JOptionPane.ERROR_MESSAGE);
					return;
				}
				...
				try {
					if (user.isCorrectPassword(oldPasswordField.getPassword())) {
						...
						JOptionPane.showMessageDialog(UpdatePasswordForm.this, "Mot de passe incorrecte !", "Erreur",
